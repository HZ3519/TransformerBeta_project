...
# sort the unique dataset indices from highest count to lowest count and select top 10k indices
dataset_indices_unique = dataset_indices_unique[np.argsort(AF_beta_strand_dataset[dataset_indices_unique, 2])[::-1]]
dataset_indices_unique = dataset_indices_unique[:10000]

# 4. build validation dataset of 10K data
validation_indices = dataset_indices_unique
...
# print highest count in validation data and lowest count in validation data
print("Highest count in validation data: " + str(AF_beta_strand_dataset[validation_indices, 2][0]))
print("Lowest count in validation data: " + str(AF_beta_strand_dataset[validation_indices, 2][-1]))
...
model_name = 'AF2_transformer_standard_validation_l12h1024_anti_l7_vtc10k'
if not os.path.exists(model_name):
	os.makedirs(model_name)
with open(model_name + '/print_message.txt', 'w') as f:
	f.write("Number of training data: " + str(X_train.shape[0]) + "\n")
	f.write("Number of unique data: " + str(len(dataset_indices_unique)) + "\n")
	f.write("Number of validation data: " + str(X_validation.shape[0]) + "\n")
	f.write("Highest count in validation data: " + str(AF_beta_strand_dataset[validation_indices, 2][0]) + "\n")
	f.write("Lowest count in validation data: " + str(AF_beta_strand_dataset[validation_indices, 2][-1]) + "\n")
