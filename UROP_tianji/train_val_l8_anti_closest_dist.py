...
# load training data
training_dict = np.load('train_l8_anti/train_dict_fold0.npy', allow_pickle=True) # <------------------------------------------------------------------------change
...
# target is a string of length 8, comp is a string of length 8, count is an integer
# combine train target and train comp into a list of length 16
training_combined = []
...
# divide the validation data into num_resources parts
validation_combined_num_split = np.array_split(validation_combined_num, num_resources)
...
for save_index, fold in enumerate(validation_combined_num_split):
	if array_index_list[counter%num_resources] == array_index:
		...
		np.save('validation_l8_anti/closest_dist_fold0_' + str(save_index) + '.npy', hamming_dist) # <------------------------------------------------------------------------change
	counter += 1
