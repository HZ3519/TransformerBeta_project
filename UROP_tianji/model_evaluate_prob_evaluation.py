...
for fold in fold_list:
	if array_index_list[counter%num_resources] == array_index:
		...
		with open('model_evaluation/prob_evaluation_{}.txt'.format(fold), 'w') as f:
		...
		# 1. sequence validation average log probability
		num_minibatch = 100 # <------------------------------------------------------------------------change
		peptide_eval = evaluate_minibatch(model_use, validation_array[:, 0], validation_array[:, 1], amino_dict, prediction_length + 2, device, num_minibatch=num_minibatch, print_info=True, output_file='model_evaluation/prob_evaluation_{}.txt'.format(fold))
		...
		with open('model_evaluation/prob_evaluation_{}.txt'.format(fold), 'a') as f:
		...
		np.save('model_evaluation/peptide_eval_{}.npy'.format(fold), peptide_eval)
