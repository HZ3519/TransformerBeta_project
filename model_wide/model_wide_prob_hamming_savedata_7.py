...
condition1 = np.nonzero(np.array([length==7 for length in BSn_data_dataset2_indices[:, 4]]))
...
for target, label in zip(target_array, label_array):
	...
	if array_index_list[counter%num_resources] == array_index:
		...
		# save to csv file
		np.savetxt('peptides_generation_data_7/{}_{}.csv'.format(target, label), final_array, delimiter=',', fmt='%s')
