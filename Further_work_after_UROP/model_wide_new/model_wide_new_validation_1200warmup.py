...
# filter out length 7 and length 8 sequences at the same time
...
working_score_tensor = torch.ones(X_train.shape[0], dtype=torch.float32)
...
lr, num_epochs, batch_size, label_smoothing = 0.0004, 10000, 6000, 0.1
...
train_seq2seq(model_wide, X_train, X_valid_len, Y_train, Y_valid_len, working_score_tensor, lr, num_epochs, batch_size, label_smoothing, amino_dict, device, model_name='model_wide_new_validation_1200warmup', warmup=1200, X_validation=X_validation, Y_validation=Y_validation, X_validation_valid_len=X_validation_valid_len, Y_validation_valid_len=Y_validation_valid_len)